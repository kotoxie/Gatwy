...
export async function verifyPasskeyRegistration(
  ...
  // Update user's MFA method to passkey
  db.run(`UPDATE users SET mfa_method = 'passkey', mfa_enabled = 1 WHERE id = ?`, [storedChallenge.user_id]);
...
export function removePasskey(passkeyId: string, userId: string): boolean {
  ...
  // If no passkeys left, disable passkey MFA method
  const remaining = getActivePasskeys(userId);
  if (remaining.length === 0) {
    db.run(`UPDATE users SET mfa_method = NULL, mfa_enabled = 0 WHERE id = ? AND mfa_method = 'passkey'`, [userId]);
  }
...
export function adminResetPasskeys(
  ...
  // Disable passkey MFA for the user
  db.run(`UPDATE users SET mfa_method = NULL, mfa_enabled = 0 WHERE id = ? AND mfa_method = 'passkey'`, [targetUserId]);
...
export async function checkInactivePasskeys(): Promise<number> {
  ...
  for (const [passkeyId, userId] of result[0].values as [string, string][]) {
    ...
    // Check if user has any remaining active passkeys
    const remaining = getActivePasskeys(userId);
    if (remaining.length === 0) {
      db.run(`UPDATE users SET mfa_method = NULL, mfa_enabled = 0 WHERE id = ? AND mfa_method = 'passkey'`, [userId]);
    }
...
// Check if user has passkey MFA enabled
export function hasPasskeyMFA(userId: string): boolean {
  const db = getDb();
  const result = db.exec(
  ...
  if (!result.length || !result[0].values.length) return false;
  return result[0].values[0][0] === 'passkey';
}
